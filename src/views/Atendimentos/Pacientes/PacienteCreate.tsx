import { Box, TextField, Typography, Grid, FormControlLabel, RadioGroup, Radio, FormHelperText } from '@mui/material';
        ...
        <Create isLoading={formLoading} saveButtonProps={customSaveButtonProps} title="Criar Paciente">
            ...
            >
                ...
                <Grid item xs={12}>
                    ...
                    <RadioGroup row>
                        {fisioterapeutas.map(({ name, value }) => (
                            <FormControlLabel
                                ...
                                control={
                                    <Radio
                                        {...register('fisioterapeuta', { required: 'Selecione um fisioterapeuta' })}
                                        value={value}
                                    />
                                }
                                label={name}
                    ...
                    <Grid item xs={12}>
                        <Typography variant="h6"  marginTop={2} fontWeight="bold">Área de Atendimento</Typography>
                        <RadioGroup
                            defaultValue="" 
                            {...register('area_atendimento', { required: "Selecione uma área de atendimento" })} 
                        >
                            <FormControlLabel value="RCP" control={<Radio />} label="RCP" />
                            <FormControlLabel value="DNM" control={<Radio />} label="DNM" />
                        </RadioGroup>
                        {errors.area_atendimento && (
                            <FormHelperText error>{errors.area_atendimento.message as string}</FormHelperText>
                        )}
                    </Grid>
                </Grid>
            </Box>
